...
export let action: ActionFunction = async ({ request }) => {
  ...
  let gun = Strapd().gun;
  let createUser = Strapd().createUser;
  let login = Strapd().login;
let errors:Err
  gun.get(`~@${username}`).once(async (user) => {
    if (!user) {
      invariant(username && password, 'string');
      const err = await createUser(username, password); // spits out either undefined or err
      if (err) {
        errors.result = err;
      } await login(username, password) // spits out encryption key
    }
  });
