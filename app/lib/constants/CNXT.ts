...
export default class CNXTag {
    ...
    async createChannel(channelName: string, isPrivate: boolean) {
        ...
        if (isPrivate) {
            ...
            if (!hash) return;
            gun.get('#public/CNXT/0').get(hash).put(JSON.stringify(channel));
            ...
            if (!peerHash) return;
            gun
                .get(`#public/CNXT/0/${channel.key}/peers`)
                .get(peerHash)
    ...
    async loadCNXT() {
        ...
        if (!gun.user().is) return;
        const loadedCNXT = {};
        const loadedCNXTList = this.CNXTList;
        const emitter = new EventEmitter();
        gun
            .user()
            .get('pchannel')
            .not((key) => {
                emitter.emit('CNXT', loadedCNXTList);
            });
        gun
            .user()
            .get('pchannel')
            .on(async (CNXT) => {
                ...
                Object.keys(CNXT).forEach(async (channelKey) => {
                    if (channelKey === '_' || loadedCNXT[channelKey]) {
                        return;
                    }
                            ...
                            .on((channel) => {
                                if (
                                    ...
                                    (channel &&
                                        channel.key &&
                                        !channel.disabled &&
                                        loadedCNXT[channelKey])
                                )
                                    return;
                                if (channel.disabled && loadedCNXT[channelKey]) {
                                    const index = loadedCNXTList
                                        .map((c) => c.key)
                                        .indexOf(channelKey);
                                    loadedCNXTList.splice(index, 1);
                                    loadedCNXT[channelKey] = false;
                                    emitter.emit('CNXT', loadedCNXTList);
                                } else if (
                                    !channel.disabled &&
                                    channel.name &&
                                    ...
                                    gun
                                        ...
                                        .get('peers')
                                        .once(async (peers) => {
                                            if (!peers || loadedCNXT[channelKey]) return;
                                            loadedCNXT[channelKey] = true;
                                            let pair = channel.pair;
                                            if (channel.isPrivate || !channel.hash) {
                                                pair = await Gun.SEA.decrypt(channel.pair, sec);
                                            }
                                            const loadedChannelIndex = loadedCNXTList.length;
                                            loadedCNXTList.push({
                                                key: channelKey,
                                                name: channel.name,
                                                userCount: 0,
                                            ...
                                            gun
                                                ...
                                                .on((newMsgs) => {
                                                    ...
                                                    if (loadedCNXTList[loadedChannelIndex]) {
                                                        loadedCNXTList[
                                                            loadedChannelIndex
                                                        ].notifCount = newCount;
                                                    }
    ...
    loadPublicCNXT() {
        ...
        const loadedCNXT = {};
        this.publicCNXTList = [];
        const loadedCNXTList = this.publicCNXTList;
        const emitter = new EventEmitter();

        return {
            on: (cb: (param: Events['publicCNXT']) => void) => {
                emitter.on('publicCNXT', cb);
                gun.get('#public/CNXT/0').on((hashMap: any) => {
                    Object.keys(hashMap).forEach((hash: string) => {
                        ...
                        gun
                            ...
                            .once((CNXTtr: string) => {
                                ...
                                const channel = JSON.parse(CNXTtr);
                                if (!channel || !channel.key || loadedCNXT[channel.key]) {
                                    return;
                                }
                                if (!channel.hash) channel.hash = hash;
                                loadedCNXT[channel.key] = true;
                                loadedCNXTList.push(channel);
                                emitter.emit('publicCNXT', loadedCNXTList);
                            });
                    });
                });
            },
    ...
    async joinPublicChannel(publicChannel: Channel) {
        ...
        if (!hash) return;
        gun
            .get(`#public/CNXT/0/${publicChannel.key}/peers`)
            .get(hash)
            .put(JSON.stringify(peerData));
            ...
            .on(async (peerInvites) => {
                ...
                Object.keys(peerInvites).forEach((peerPub) => {
                    ...
                    gun
                        ...
                        .get(peerPub)
                        .on(async (CNXT) => {
                            if (!CNXT || CNXT === 'disabled') return;
                            Object.keys(CNXT).forEach(async (channelKey) => {
                                const channel =
                                    typeof CNXT[channelKey] === 'string' &&
                                        CNXT[channelKey] !== 'disabled'
                                        ? JSON.parse(CNXT[channelKey])
                                        : CNXT[channelKey];
                                if (
                                    channelKey === '_' ||
                                    !channel ||
                ...
                Object.keys(peerMsgs).forEach((time) => {
                    ...
                    path.get(time).on(async (msgDataString) => {
                        ...
                        if (
                            ...
                            const CNXTec = await (Gun.SEA as any).secret(
                                channel.key,
                                channel.pair,
                            );
                            msgToLoad = await Gun.SEA.decrypt(msgData.msg, CNXTec);
                        }
